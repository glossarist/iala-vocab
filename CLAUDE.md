...
1. **`scrape_sections.rb`** — fetches `Chapter_Index` via MediaWiki CategoryTree, writes `reference-docs/sections/section-tree.json`. The 13 top-level sections (ids `0`–`12`) are hard-coded in `TOP_LEVEL_SECTIONS`; subsection ids (`1.1`, `1.2`, …) are discovered from the tree.
2. **`scrape_edition.rb "<Category_Name>"`** — fetches every category member's parsed HTML + raw wikitext + categories + langlinks, caches per-page JSON under `reference-docs/editions/<edition>/pages/`, and writes `index.json`. Category names map to edition ids via `EDITION_MAP`:
   - `IALA_Dictionary_1970-89_Edition` → `iala-1970-89`
   - `IALA_Dictionary_2023_Revision` → `iala-2023`
3. **`scrape_translations.rb`** — pulls French (`Classement_alphabétique` → `fra`) and Spanish (`Indice_alfabeto_Español` → `spa`) category members into `reference-docs/translations/{fra,spa}/`.
4. **`generate_register.rb`** — emits `datasets/iala-{1970-89,2023}/register.yaml` by combining the shared `section-tree.json` with edition-specific `year`/`urn`/`status`. Key order is forced via `format_yaml`.
5. **`transform_iala.rb <edition>`** — turns each cached page into a Glossarist v3 multi-doc YAML at `datasets/<edition>/concepts/<termid>.yaml`. See "Concept YAML schema" below.
6. **`link_editions.rb`** — matches concepts across editions by `termid`, normalizes English definitions (`normalize_text`), and injects `related[]` entries on both sides. Writes `reference-docs/cross-edition-report.json` with the counts.
7. **`add_year_tags.rb`** — maps MediaWiki categories (e.g. `IALA Dictionary 2015 Revision`, `Approved by DWG`) to `dates[]` and `approval` on the managed concept.
8. **`download_images.rb`** — scrapes `src="…/images/…"` URLs out of cached page HTML, downloads to `public/images/iala/`, filters UI icons (`Geographylogo.png`, `Npx-` prefixed, <1KB). Writes `reference-docs/image-map.json`.
9. **`audit_iala.rb`** — validates termids are present/unique, `terms[]` is non-empty, `definition[]` has `content`. Exits non-zero on errors — GH Pages build should fail closed on this.

## MediaWiki API client

`scripts/iala_api.rb` is the only network surface. Every request is cached by `MD5(canonical_url)` under `reference-docs/api-cache/<hash>.json`. **Once cached, the cache is the source of truth — edits to upstream MediaWiki will not be picked up until you delete the cache file.** To force re-fetch, delete the relevant cached JSON (or all of `reference-docs/api-cache/`).
...
- **`reference-docs/`** — cached MediaWiki API responses, `index.json`, `section-tree.json`, `cross-edition-report.json`, `image-map.json`, translations. Required to re-run transform/link/audit without hitting the network. Treat as data provenance, not build output.
